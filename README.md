...
<img src="https://github.com/user-attachments/assets/7d3309fe-05b1-4a31-994f-31260d37ea21" alt="Daily Fragment Hero" />
...
<img src="https://github.com/user-attachments/assets/ba45c3ff-d2af-405e-a13e-2567e192c884" alt="Calendar home" />

**公開ポートフォリオ `/{handle}`**

<img src="https://github.com/user-attachments/assets/673db8dc-6b4c-4ee8-8fa0-cb3d8afa52ad" alt="Public portfolio" />
...
- **scarlet7-template** (by NOP / shogo): 初期テンプレート（Next.js + Supabase + Amplify の足場）を提供いただきました。テンプレートの README は [元リポジトリ](https://github.com/) を参照
- **本作品の担当範囲**:
  - 作品コンセプト・世界観・命名の設計
...
詳細なセットアップ手順は scarlet7-template の README を参照。要点のみ：

```bash
# 依存関係インストール
npm install

# ローカル Supabase 起動（Docker 必須）
npm run db:start

# .env を設定（テンプレートの README 参照）
...
| `npm run build` | 本番ビルド |
| `npm run check` | ESLint + TypeScript 型チェック |
| `npm run db:start` | ローカル Supabase 起動 |
| `npm run db:reset` | ローカル DB リセット |
| `npm run db:diff <name>` | Migration 生成 |
